                ...
                bcrypt.hash(req.body.Password, null, null, function(err, hash){
                    ...
                    user.save((err, userSave) => {
                        if(err)
                        {
                            ...
                            res.status(201).send({message: 'Usuario Guardado verifique email'})
...
function login(req, callback){
    userNride.findOne({Email: req.Email.toLowerCase()},(err,user) => {
        if(err)
        {
        ...
        else
        {
            if(!user)
            {
                callback(404,{ message: 'Email no encontrado' })
            }
            else
            {
                //Comparar contraseña
                bcrypt.compare(req.Password, user.Password, function(err, check)
                {
                    ...
                    {   //verifica si el email ya es valido
                        ...
                        {//contraseña incorrecta
                            callback(404,{ message: 'Email aun no se ha activado'})
                        }
                    }
                    else{
                        //contraseña incorrecta
                        callback(404,{ message: 'Contraseña incorrecta'})
                    }
                });
            }
        }
...
function verifiyToken(req, callback){
    ...
    userNride.findOne({token:req}, function (error, user) {
        if(error){
            res.status(500).send({ message: `Error al buscar token ${error}` })
        }else if(user)
        {
            user.set({emailValidate:true})
            ...
            {
                callback(200,{message:'Usuario activado'})
...
function resetPassword(req, res){
    userNride.findOne({Email:req.body.Email}, (err,user) => {
        if(err)
        {
            res.status(500).send({ message: `Error al encontrar el email en la base de datos: ${err}` })
        }
        else if(user) {
            ...
            if(user.emailValidate)
            {
                utl.generatePassword(function (err,resul) {
                    if(err){
                        res.status(500).send({ message: `Error email no valido: ${err}` })
                    }
                    else{
                        ...
                        bcrypt.hash(temporalPass, null,null, function(err, hash){
                            ...
                            user.save(function(e,updUser){
                                if(e)
                                {
                                    res.status(500).send({ message: `Error al guardar contraseña: ${err}` })
                                ...
                                else
                                {
                                    ...
                                    res.status(201).send({message: 'Contraseña Cambiada'})
            ...
            userNride.findById((result.sub*1.0), (err, user)=> {
                ...
                else
                {
                    ...
                    user.save(function(e, usr){
                        ...
                        {
                            console.log(e);
                        }else{console.log('Se guardo session null al eliminar')}
